...
//                                $sql2 = "INSERT INTO admin (username,password,email,created_date,verification_key,admin_group,name,address,address2,city,state,zip) VALUES ($username, $password, $email, $now, $verification_key, $admin_group, $name, $address, $address2, $city, $state, $zip)";
                                $sql2 = "INSERT INTO admin (username,password,email,created_date,verification_key,name) VALUES ($username, $password, $email, $now, $verification_key, $name)";
                                $this->db->query($sql2);
                                return TRUE;   
                        }
...
//                        if (!isset($postData["zip"]) || empty($postData["zip"])) { $zip = "''";} else { $zip = $this->db->escape(strip_tags($postData["zip"]));}
                        ...
                        if ($query->num_rows() > 0) {
                                if ($pass == 0) {
                                    $sql = "UPDATE admin SET email = $email, name = $name WHERE id = ".$this->db->escape($query->row()->id);
//                                    $sql = "UPDATE admin SET email = $email, name = $name
//									, admin_group = $admin_group
//									, address = $address, address2 = $address2, city = $city, state = $state, zip = $zip WHERE id = ".$this->db->escape($query->row()->id);
                                    $this->db->query($sql);
                                    return TRUE;
                                } else {
